                        ...
                        db.collection("chapters").document(chapterUID as! String).collection("events").document("events").getDocument { (chapterEventDocument, error) in
                            if let chapterEventData = chapterEventDocument!.data() {
                                if (chapterEventData[eventCode] != nil) {
                                    ...
                                    if let meetings = documentData["meetings"] as! NSMutableDictionary? {
                                        if meetings.value(forKey: eventCode) == nil {
                                            db.collection("chapters").document(chapterUID as! String).collection("students").document("student").setData([self.uid!:["meetings":[eventCode:"attended"]]], merge: true){err in
                                                if let err = err {
                                                    print("Error writing request document: \(err)")
                                                } else {
                                                    print("Document succesfully written")
                                            ...
                                            db.collection("chapters").document(chapterUID as! String).collection("meetings").document("meetings").setData([eventCode:["attendees":[self.uid:"attended"]]], merge: true){err in
                                                if let err = err {
                                                    print("Error writing document: \(err)")
                                                } else {
                                                    print("Document succesfully written")
                                            ...
                                            db.collection("students").document(self.uid!).setData(["meetings" : [eventCode : "attended"]], merge: true){err in
                                                if let err = err {
                                                    print("Error writing request document: \(err)")
                                                } else {
                                                    print("Document succesfully written")
                                            ...
                                            let alert = UIAlertController(title: "You've already signed in!", message: "You have already signed in to this meeting.", preferredStyle: .alert)
                                            alert.addAction(UIAlertAction(title: "Ok", style: .default, handler: {_ in
                                                let storyboard = UIStoryboard(name: "StudentViews", bundle: nil)
                                                let viewController = storyboard.instantiateViewController(identifier: "StudentHomeViewController") as! StudentHomeViewController
                                                viewController.uid = self.uid
